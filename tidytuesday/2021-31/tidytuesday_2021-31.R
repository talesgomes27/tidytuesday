...
country_colors <- cbind(c("#FFFFFF", "#FF0000"), c("#ffffff", "#012169"),
...
comp_plot <- ggpubr::ggarrange(myplots[[1]], myplots[[2]], myplots[[3]], myplots[[4]],
                  labels = c("Canada", "China", "USA", "Brazil"),
                  ncol = 2, nrow = 2,
                  font.label = list(size = 14,
                                    color = "darkgray", face = "bold"))

  ggpubr::annotate_figure(comp_plot,
                          top = ggpubr::text_grob("Summer olympics male to famale athletes proportion from 1948 to 2016",
                          color = "darkgray", face = "bold", size = 20),
                          bottom = ggpubr::text_grob("@talesgomes2709 | #tidytuesday | source: kaggle",
                                                     color = "darkgray", face = "bold", size = 18))
...
fig_name <- str_c("tidytuesday/2021-31/fig/",
                  olympics_country$region[1], "_participation_per_sex.png")
ggsave(fig_name,
       scale = 1,
       dpi = 600,
       width = 45,
...
ggpubr::annotate_figure(comp_plot,
                        top = ggpubr::text_grob("Summer olympics participation by gender from 1948 to 2016",
                                                color = "darkgray", face = "bold", size = 24),
                        bottom = ggpubr::text_grob("@talesgomes2709 | #tidytuesday | source: kaggle",
                                           color = "darkgray", face = "bold", size = 18))

#Salvando iagens unificadas
ggsave("tidytuesday/2021-31/fig/Comp_participation_per_sex.png",
       scale = 1,
       dpi = 600,
       width = 90,
       height = 50,
...
for(i in 1:4){
  ...
  country_title <-  str_c("Summer olympics medals by gender",
                          olympics_country$region[1],
                          "from 1964 to 2016", sep = " ") 
#Filtrando os dados
...
fig_name <- str_c("tidytuesday/2021-31/fig/",
                  olympics_country$region[1], "_medal_per_gender.png")
ggsave(fig_name,
       scale = 1,
       dpi = 600,
       width = 45,
...
ggpubr::annotate_figure(comp_plot,
                        top = ggpubr::text_grob("Summer olympics medals by gender from 1948 to 2016",
                                                color = "darkgray", face = "bold", size = 24),
                        bottom = ggpubr::text_grob("@talesgomes2709 | #tidytuesday | source: kaggle",
                                                   color = "darkgray", face = "bold", size = 18))

#Salvando iagens unificadas
ggsave("tidytuesday/2021-31/fig/Comp_medals_per_sex.png",
       scale = 1,
       dpi = 600,
       width = 90,
       height = 50,
