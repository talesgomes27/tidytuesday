...
::: {.fullwidth}
Este trabalho foi feito com base nos dados históricos olímpicos modernos (fonte:[kaggle](https://www.kaggle.com/heesoo37/120-years-of-olympic-history-athletes-and-results)), tendo como objetivo realizar uma comparação do brasil com a média olimpica e os maiores medalistas americanos (Estados unidos e Canadá) em relação à participação feminina de 1948 até 2016. Para isso foram feitas análises por participação por esporte praticado e ano olímpico, bem como por número de medalha esporte e ano olímpico. Canadá e Estados Unidos foram escolhidos para comparação por estarem no continente americano, possuírem tamanhos similares ao do Brasil, além do fato de terem participado de todas as edições olímpicas no período estudado. 
:::
...
new_title <- gtable(unit(c(1, 13, 1), "in"), unit(0.9, "in")) |>
  ...
  gtable_add_grob(
    textGrob(
      label = "Total olympic participation from 1948 to 2017",
      x = unit(0, "npc"),
      just = "left",
      gp = gpar(fontsize = 47)
    ),
...
```{r Análise países - participação por ano}
...
for (i in 1:3) {
  ...
  new_title <-
    gtable(unit(c(0.9, 12.4, 0.9), "in"), unit(0.5, "in")) |>
...
```{r FIgura Participação média por ano, fig.width = 18, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Fígura participação Canada por ano, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Fígura participação EUA por ano, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Fígura participação Brasil por ano, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
new_title <- gtable(unit(c(1, 13, 1), "in"), unit(0.9, "in")) |>
  ...
  gtable_add_grob(
    textGrob(
      label = "Total olympic medals per gender from 1948 to 2017",
      x = unit(0, "npc"),
      just = "left",
      gp = gpar(fontsize = 41)
    ),
...
for (i in 1:3) {
  ...
  country_title <- str_c("Total olympic medals per gender from 1948 to 2017 in",
                         olympics_country$region[1],
                         sep = " ")
...
```{r Figura Media medalhas por ano, fig.width = 18, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura Canada medalhas por ano, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura EUA medalhas por ano, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura Brasil medalhas por ano, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
new_title <-
  gtable(unit(c(0.9, 9.3, 0.9), "in"), unit(0.9, "in")) |>
  gtable_add_grob(grobs = Olympic_rings, t = 1, l = 1) |>
  gtable_add_grob(
    textGrob(
      label = "Total olympic participation from 1948 to 2017",
...
```{r Analise Paises - participação por esporte}
...
```{r Figura Media participação  por esportes, fig.width = 18, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura participação Canada  por esportes, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}

grid.draw(list_fig_sex_sport[[1]])

```

```{r Figura participação EUA  por esportes, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura participação Brasil  por esportes, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Análise países - medalhas por esporte}
...
```{r Figura Media medalhas por esporte, fig.width = 18, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura medalhas Canada por esporte, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura medalhas EUA por esporte, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
...
```{r Figura medalhas Brasil por esporte, fig.width = 22, fig.height = 14, fig.fullwidth = TRUE}
